...
class MAE(nn.Module):
    ...
    def forward(self, x, return_image=False):
        ...
        if return_image:
            ...
            masked_x = torch.cat(
                [shuffled_x[:, :sel_length, :], 0.5 * torch.ones(batch_size, msk_length, out_chans).to(x.device)],
            ...
            # recon = all_x[:, 1:, :].gather(dim=1, index=unshuffle_indices[:, :, None].expand(-1, -1, out_chans))
            #                        .reshape(batch_size, -1, args.in_chans, args.patch_size**2)
            # reshaped_x = x.reshape(batch_size, -1, args.in_chans, args.patch_size**2)
            # recon = recon * (reshaped_x.var(dim=-1, unbiased=True, keepdim=True).sqrt() + 1e-6) +
            #                   reshaped_x.mean(dim=-1, keepdim=True)
